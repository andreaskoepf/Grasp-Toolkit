classdef Hand < handle
    properties(SetAccess=protected)
        ...
        PhysicsHandle
        ClassIndex=int32(3);
        MethodIndexMap=Hand.getMethodIndexMap;
    end
...
%             end
        ...
        function setPhysicsHandle(h)
            ...
            % adding all bodies to the articulated body in the simulation
            for n=1:length(bodies);
                h.addBody(bodies{n});
            end
        ...
        function destroyPhysicsHandle(h)
            ...
            if(h.InPhysicsMode)
                cId=h.ClassIndex;
                mId=h.MethodIndexMap('Destructor');
                PhysicsWorld.PhysicsFunction(cId,mId,h.PhysicsHandle);
            end
    ...
    methods (Access=protected)
        ...
        function startUpdate(h)
            ...
            if(h.InPhysicsMode)
                cId=h.ClassIndex;
                mId=h.MethodIndexMap('startUpdate');
                PhysicsWorld.PhysicsFunction(cId,mId,h.PhysicsHandle);
            end
        end
        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function stopUpdate(h)
            % this function is called once each body's transform has been
            % updated.              
            
            if(h.InPhysicsMode)
                cId=h.ClassIndex;
                mId=h.MethodIndexMap('stopUpdate');
                PhysicsWorld.PhysicsFunction(cId,mId,h.PhysicsHandle);
            end
        end
