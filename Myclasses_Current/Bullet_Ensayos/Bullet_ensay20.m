...
%% performing contact test
% must pause simulation in order to prevent run time errors from occurring due
% to multiple processes trying to access the same resource
fingerLink = h(1).Fingers(1).Links(1);
[bool,contactResults] = fingerLink.collisionPairTest(grabbedBox);
