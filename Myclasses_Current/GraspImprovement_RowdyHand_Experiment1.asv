...
obj = Sphere(4);
%%
objectTransform = Geometry.translation('zxy',[-2 8 -4]);
objectColor = [1 1 0];
palmStartTransform = Geometry.translation('zyx',[2 -8 2])*Geometry.rotation('x',pi/2);
...
obj.scale(2);
%%
objectTransform = Geometry.translation('zxy',[-2 8 0]);
objectColor = [135  206 250]/255;
palmStartTransform = Geometry.translation('zyx',[5 -8 2])*Geometry.rotation('x',pi/2);
palmEndTransform = palmStartTransform*Geometry.translation('z',-10);
testNo = 3;
...
%%
hand.Fingers(1).JointValues = [pi/2 pi/40 pi/6 pi/6];
hand';
...
jc1.setJointValues([0 0 0 0]);
jc2.setJointValues([pi/2 0 pi/3 pi/2]);
...
jc1.setJointValues([0 -pi/10 0 0]);
jc2.setJointValues([-pi/10 pi/6 pi/8 pi/2]);
