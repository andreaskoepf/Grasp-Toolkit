...
% point for the closest point to the origin in the current polytope.  It
% also prints time and final expanded polytope results.
%   [d,resultDetails] = PenetrationDepth(A,E)
%   inputs:
...
%   ouputs:
%
%       resultDetails : Optional return structure that contains all result
%           details:
%           PenetrationDepth:
%               Shortest Distance between convex Set A and the origin.
%           ClosestPoint:  
%               Closest point to the origin in A.
%           ClosesFacet:
%               
%           NumOfIterations:
%               
%           TotalTime:
%               
%           NumFacetsOfExpandedPolytope:
%               
%           ExpandedPolytope:
%                   Struct with fields "vertices" and "faces" contain the
%                   corresponding vertices and faces arrays that define the
%                   n - dimensional expanded polytope.
%
%
function [d,varargout]=PenetrationDepthShow(A,varargin)
...
while(NumIter<100)
    ...
    %  obtaining new faces
    [facesNew] = setdiff(K,facesOld,'rows');
    %facesNew = K;
...
switch nargout
    case 2
        ...
        resultDetails = struct('PenetrationDepth',[],'ClosestPoint',[]...
            ,'ClosestFacet',[],'NumberOfIterations',[],'TotalTime',[]...
            ,'NumFacetsOfExpandedPolytope',[],'ExpandedPolytpe',[]);
        
        resultDetails.PenetrationDepth = d;
        resultDetails.ClosestPoint = pk;
        resultDetails.ClosestFacet = facesNew(n,:);
        resultDetails.NumberOfIterations = NumIter;
        resultDetails.TotalTime = totalTime;
        resultDetails.NumFacetsOfExpandedPolytope = size(K,1);
        
        % constructing expanded polytope
        expdPolytope=struct('vertices','faces');
        expdPolytope.vertices=Ak;
        expdPolytope.faces=K;
        
        resultDetails.ExpandedPolytope = expdPolytope;
        
        varargout{1} = resultDetails;
