...
C2A_Model* C2AUtilities::create_C2A_Model(double *vertices,int *faces,int numVertices,int numFaces)
{
	...
	for(int i = 0;i<numFaces;i++)
	{
		i1 = faces[3*i];	// c arrays are zero based
		i2 = faces[3*i+1];
		i3 = faces[3*i+2];
...
C2A_Model* C2AUtilities::create_C2A_Model(const mxArray *meshData)
{
	...
	return create_C2A_Model(meshData,localTransform);
}
