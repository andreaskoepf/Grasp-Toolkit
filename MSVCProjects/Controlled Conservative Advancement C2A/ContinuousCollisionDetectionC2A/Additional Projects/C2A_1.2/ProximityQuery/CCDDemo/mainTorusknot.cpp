...
{  
	...
	case 1:
		...
		MVtoOGL(oglm,xformframes[0][step1].R,xformframes[0][step2].T);
		glPushMatrix();
		glMultMatrixd(oglm);
		object1_drawn->Draw();             // do gl rendering
		glPopMatrix();                    // restore transform
...
void main(int argc, char **argv)
{
  // init glut
	...
	object1_tested->Vertex = new Coord3D[nvertics];//changed in 6.10
	...
	for (i = 0; i < nvertics; i++)
	{
		...
		p[3*i+1] = (PQP_REAL)b;
		p[3*i+2] = (PQP_REAL)c;
		object1_tested->Vertex[i].Set_Value(a,b,c);//changed in 6.10
	}
	...
	object2_tested->Vertex = new Coord3D[nvertics];//changed in 6.10
	...
	for (i = 0; i < nvertics; i++)
	{
		...
		p[3*i+1] = (PQP_REAL)b;
		p[3*i+2] = (PQP_REAL)c;
		object2_tested->Vertex[i].Set_Value(a,b,c);//changed in 6.10
