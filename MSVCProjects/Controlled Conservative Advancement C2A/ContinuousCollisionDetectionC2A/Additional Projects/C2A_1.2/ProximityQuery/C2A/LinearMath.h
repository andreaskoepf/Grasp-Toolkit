...
class Quaternion {
  public:
    ...
    inline Real Angle( ){return 2.0 * acos(val[3]);}
...
class Matrix3x3 {
  public:
	...
	inline void Set_Value_exp(Coord3D omiga, Real theta);
...
inline void Matrix3x3::Set_Value_exp(Coord3D omiga, Real theta)
{
	Real temp[3][3];
	temp[0][0]=0;temp[0][1]=-omiga.Z();temp[0][2]=omiga.Y();
	temp[1][0]=omiga.Z();temp[1][1]=0;temp[1][2]=-omiga.X();
	temp[2][0]=-omiga.Y();temp[2][1]=omiga.X();temp[2][2]=0;
	Matrix3x3 temp1(temp);
	Matrix3x3 temp2(temp);
	Matrix3x3 temp3(temp);
	temp1*=sin(theta);
	
	Real temp2_val[3][3],temp3_val[3][3],temp_sq[3][3];
	temp2.Get_Value(temp2_val);
	temp3.Get_Value(temp3_val);
	MxM(temp_sq,temp2_val,temp3_val);
	temp2.Set_Value(temp_sq);
	temp2*=(1-cos(theta));
	temp3.Identity( );
