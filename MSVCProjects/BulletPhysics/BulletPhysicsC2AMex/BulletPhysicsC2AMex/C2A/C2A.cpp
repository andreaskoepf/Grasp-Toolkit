						  ...
						  double delta)
{
  ...
  { 
	...
	if (dTri <= res->distance)
	{
        ...
        PQP_REAL mint =  (dTri- 0.1 * delta)/(motionbound1+motionbound2);
		if (mint<0.0)
		{
			mint=0.0;
		}
  ...
  if (d1!=0.0)
  {
	  ...
	  mintb         = (d1 - 0.1 * delta)/(motion_bound1+motion_bound2);
	  if (mintb <= 0)
	  {
		  mintb = 0.0;
	  }
  ...
  if (d2!=0.0)
  {
	  ...
	  minta         = (d2 - 0.1 * delta)/(motion_bound1+motion_bound2);
						   ...
						   int b2)
{
  ...
  if (l1 && l2)
  {
    ...
    if (d < res->distance) 
    {
			...
			int TriangleID_B = -o1->child(b2)->first_child - 1;
					   ...
					   int qsize)
{
	...
	PQP_CollideResult res_col;



	PQP_Collide( &res_col, res->R_toc, res->T_toc, o1,
		R2, T2,o2, 2);

	if(res_col.num_pairs>0)
	{		
		return true;//collision at time 1.0
	}
...
void Contact_normal_new(C2A_TimeOfContactResult *res,C2A_Model *o1,C2A_Model *o2, Transform moving_transform, Transform object_2_pose)
{
	...
	int number_of_contact = res->num_contact;



	// feature construction
	ContactFListIterator ContactFit;
	ContactFit = res->cont_l.begin();
	...
	for (int jj = 0 ; jj < number_of_contact ; jj++) 
	{
		...
		if( (*ContactFit).FeatureType_A==1 ) {
			pb1=o1->Vertex[(*ContactFit).FeatureID_A[0]];
			pb1^=moving_transform;
		}
		else if( (*ContactFit).FeatureType_A==2 ) {
			pb1=o1->Vertex[(*ContactFit).FeatureID_A[0]];
			pb1^=moving_transform;

			pb2=o1->Vertex[(*ContactFit).FeatureID_A[1]];
			pb2^=moving_transform;
		}
			...
			pb3=o1->Vertex[(*ContactFit).FeatureID_A[2]];
			pb3^=moving_transform;
		}

		if( (*ContactFit).FeatureType_B==1 ) {
			pa1=o2->Vertex[(*ContactFit).FeatureID_B[0]];
			pa1^=(object_2_pose);
		}
			...
			pa2=o2->Vertex[(*ContactFit).FeatureID_B[1]];
			pa2^=(object_2_pose);
		}
			...
			pa3=o2->Vertex[(*ContactFit).FeatureID_B[2]];
			pa3^=(object_2_pose);
		}
		//edge-edge
		if ( cl == 8 ) {
...
//	C2A_TimeOfContactResult dres;
	...
	PQP_REAL d_delta = 0.001*obj1_tested->radius;
	...
	PQP_Collide(&res_col, R1, T1, obj1_tested,
		R2,T2,obj1_tested, 2);
	...
	if(!dres.collisionfree)
	{
		...
		Contact_normal_new(&dres,obj1_tested,obj2_tested,trans0,trans1);//changed in 6.10
...
void C2A_Model::ComputeCenterOfMass()
{
    ...
    for( i = 0; i < num_tris; i++ ) 
	{
		...
		VmV(E0, ((C2A_Tri*)tris)[i].p1, ((C2A_Tri*)(tris))[i].p2);
		VmV(E1, ((C2A_Tri*)(tris))[i].p1, ((C2A_Tri*)(tris))[i].p3);
		VcrossV(S, E0, E1);
		area_x2=sqrt(S[0]*S[0] + S[1]*S[1] + S[2]*S[2]);
        total_area += area_x2;
		
        com[0] += (((C2A_Tri*)(tris))[i].p1[0] + ((C2A_Tri*)(tris))[i].p2[0] + ((C2A_Tri*)(tris))[i].p3[0]) * area_x2;
        com[1] += (((C2A_Tri*)(tris))[i].p1[1] + ((C2A_Tri*)(tris))[i].p2[1] + ((C2A_Tri*)(tris))[i].p3[1]) * area_x2;
		com[2] += (((C2A_Tri*)(tris))[i].p1[2] + ((C2A_Tri*)(tris))[i].p2[2] + ((C2A_Tri*)(tris))[i].p3[2]) * area_x2;
    }
...
void C2A_Model::ComputeRadius()
{
    ...
    for( i = 0; i < num_tris; i++ )
	{
		...
		p[0] = ((C2A_Tri*)(tris))[i].p1[0];
		p[1] = ((C2A_Tri*)(tris))[i].p1[1];
		p[2] = ((C2A_Tri*)(tris))[i].p1[2];		
		VmV(pr, p, com);
		d = Vlength(pr);		
		if( d > radius ) radius = d;

		p[0] = ((C2A_Tri*)(tris))[i].p2[0];
		p[1] = ((C2A_Tri*)(tris))[i].p2[1];
		p[2] = ((C2A_Tri*)(tris))[i].p2[2];		
		VmV(pr, p, com);
		d = Vlength(pr);	
		if( d > radius ) radius = d;
		
		p[0] = ((C2A_Tri*)(tris))[i].p3[0];
		p[1] = ((C2A_Tri*)(tris))[i].p3[1];
		p[2] = ((C2A_Tri*)(tris))[i].p3[2];		
		VmV(pr, p, com);
		d = Vlength(pr);	
		if( d > radius ) radius = d;
    }
