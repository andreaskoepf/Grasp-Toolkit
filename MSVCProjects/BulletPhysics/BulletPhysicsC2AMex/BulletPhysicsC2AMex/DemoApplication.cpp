...
/* List of changes
1 - Line 352 : added isSimulationRunning=false
*/
...
void DemoApplication::myinit(void)
{
	...
	GLfloat light_position0[] = { 1.0, 1.0, 100.0, 1.0};
	GLfloat light_position1[] = { -1.0, -1.0, -100.0, 1.0 };
...
void DemoApplication::showProfileInfo(int& xOffset,int& yStart, int yIncr)
{
...

	{
		...
		{
			sprintf_s(blockTime,"--- Profiling: %s (total running time: %.3f ms) ---",	m_profileIterator->Get_Current_Parent_Name(), parent_time );
			displayProfileString(xOffset,yStart,blockTime);
			yStart += yIncr;
			sprintf_s(blockTime,"press number (1,2...) to display child timings, or 0 to go up to parent" );
			displayProfileString(xOffset,yStart,blockTime);
			yStart += yIncr;
...
//		m_dynamicsWorld->getDebugDrawer()->drawAabb(aabbMin,aabbMax,btVector3(1,1,1));
		...
		case	2:	
			m_shapeDrawer->drawOpenGL(m,colObj->getCollisionShape(),wireColor*0.3,0,aabbMin,aabbMax);
			//std::cout<<"\nCalled rederme("<<pass<<")"<<std::endl;
			break;
		}
	}
}
